   ...
   /* echo $kalender;*/
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form>
              <div class="form-group">
                  <input class="form-control" type="hidden" id="id_rapat">
              </div>
              <div class="form-group">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" onclick="submit_edit_rapat()">Save Changes <i class='fa fa-plus fa-sm'></i></button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal fade" id="modal_add_events" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="title_modal_add_events">Tambah Data Rapat </h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick="close_events()">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" onclick="submit_rapat()">Simpan</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal fade" id="modal_events" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <!-- <div class="modal-footer">
            <button type="button" class="btn btn-primary" onclick="modal_add_events()">Tambah Data Rapat <i class='fa fa-plus fa-sm'></i></button>
          </div> -->
        </div>
      </div>
    </div>
    ...
    function month_year_now(konteks){
      ...
      $.ajax({
      url : "<?php echo site_url('Admin/Kalender/Kalender_rapat/change_month_year')?>?month="+month+"&years="+years,
      type: "GET",
      dataType: "JSON",
    ...
    function myFunction(id) {
      ...
      check_events(tanggal_full);
			// document.getElementById("title_modal_events").innerHTML = "Rapat @"+tanggal+" "+document.getElementById('title_month').innerText+" "+document.getElementById('title_year').innerText;
			document.getElementById("title_modal_events").innerHTML = "Rapat pada tanggal "+tanggal+" "+document.getElementById('title_month').innerText+" "+document.getElementById('title_year').innerText;
    ...
    function check_events(tanggal){
      $.ajax({
          url : "<?php echo site_url('Admin/Kalender/Kalender_rapat/check_events')?>/" + tanggal,
          type: "POST",
          dataType: "JSON",
          success: function(data)
          {          
    ...
    function delete_rapat(id_rapat){
      $.ajax({
      url : "<?php echo site_url('Admin/Kalender/Kalender_rapat/delete_rapat')?>?id_rapat="+id_rapat,
      type: "GET",
      dataType: "JSON",
      success: function(data)
      {          
    ...
    function edit_rapat(id_rapat){
      $.ajax({
      url : "<?php echo site_url('Admin/Kalender/Kalender_rapat/edit_rapat')?>?id_rapat="+id_rapat,
      type: "GET",
      dataType: "JSON",
      success: function(data)
      {          
          document.getElementById("title_modal_edit_events").innerHTML = "Edit "+document.getElementById("title_modal_events").innerHTML;
          document.getElementById("form_deskripsi_edit").value = data.deskripsi_rapat;
          document.getElementById("time_edit").value = data.time;
          document.getElementById("id_rapat").value = id_rapat;
          $('#modal_events').modal('hide');
    ...
    function submit_edit_rapat(){
      ...
      var id_rapat = document.getElementById('id_rapat').value;
      var tanggal_full= document.getElementById('title_year').innerText+months[document.getElementById('title_month').innerText]+pad(tanggal);
      var deskripsi = document.getElementById("form_deskripsi_edit").value;
      $.ajax({
        url : "<?php echo site_url('Admin/Kalender/Kalender_rapat/submit_edit_rapat')?>?deskripsi="+deskripsi+"&tanggal="+tanggal_full+"&time="+time+"&id_rapat="+id_rapat,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {          
    ...
    function submit_rapat(){
      ...
      $.ajax({
        url : "<?php echo site_url('Admin/Kalender/Kalender_rapat/submit_rapat')?>?deskripsi="+deskripsi+"&tanggal="+tanggal_full+"&time="+time,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {          
    ...
    function flash_data_delete(){
      document.getElementById("flashdata").innerHTML = "Data rapat berhasil ditambahkan";
                $('.flashdata').fadeIn(1000);
                setTimeout(function(){
                  $('.flashdata').fadeOut(1000);
                },3000);
    ...
    function reload_calendar(konteks) {
      if (konteks=="submit") {
        ...
        document.getElementById("flashdata").style.backgroundColor = "#53942a";
          flash_data("Data rapat berhasil diubah !");
      }
      else{
        document.getElementById("flashdata").style.backgroundColor = "red";
          flash_data("Data berhasil dihapus !");
      }
